                ...
                self.history.append(
                    {
                        "type": e.get("type", "unknown"),
                        "files": [
                            tuple(p) if isinstance(p, list) and len(p) == 2 else (str(p), "")
            ...
            for src, dst in file_pairs:
                try:
                    ...
                    if src_p.exists() and self._copy_is_pristine(src_p, dst_p):
                        ...
                        self._log(
                            f"⚠️ 撤销导入：副本 {dst_p.name} 已被修改，已隔离到 .trash_backup（未直接删除，可找回）",
                            "warning",
                        )
                    else:
                        src_p.parent.mkdir(parents=True, exist_ok=True)
                        shutil.move(str(dst_p), str(src_p))
                        self._log(f"↩️ 撤销导入: 源文件已不存在，副本已移回 {src_p}", "warning")
                    success_count += 1
                except Exception as e:
