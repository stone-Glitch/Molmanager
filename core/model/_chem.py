...
class ChemMixin:
    ...
    def collect_results(self) -> list[dict]:
        ...
        for summary_json in self.work_dir.rglob("*_summary.json"):
            ...
            try:
                if self._touches_protected(summary_json.relative_to(self.work_dir)):
                    continue
            ...
            try:
                ...
                base = (
                    base_with_suffix[: -len("_summary")] if base_with_suffix.endswith("_summary") else base_with_suffix
                )
    ...
    def compare_directories(self, left: str | Path, right: str | Path) -> dict:
        ...
        for name in left_names & right_names:
            ...
            if ls != rs or lm != rm:
                diff_content.append(
                    {"name": name, "left_size": ls, "left_mtime": lm, "right_size": rs, "right_mtime": rm}
                )
        only_left.sort(key=lambda x: x["name"])
        only_right.sort(key=lambda x: x["name"])
        diff_content.sort(key=lambda x: x["name"])
        return {"only_left": only_left, "only_right": only_right, "diff_content": diff_content}
    ...
    def copy_from_left_to_right(self, names: list[str], left, right):
        ...
        if self.is_protected(left) or self.is_protected(right):
            raise ValueError(f"同步目录命中受保护目录（.backup/.trash_backup），拒绝操作: {left!r} / {right!r}")
