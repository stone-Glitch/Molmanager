...
class DiffSyncDialog(ThemedDialog):
    """⚖️ 两工作目录差异比较 + 一键同步。"""

    dialog_title = "⚖️ 两工作目录差异比较 + 一键同步"
    initial_size = (800, 520)
    ...
    def _build_body(self):
        ...
        def _do_compare():
            ...
            if not left or not right:
                messagebox.showwarning("提示", "请先填写左右目录", parent=self)
                return
        ...
        def _require_dirs():
            ...
            right = right_dir_var.get().strip()
            if not left or not right:
                messagebox.showwarning("提示", "请填写左右目录", parent=self)
            return left, right
        ...
        def _only_left_copy_right():
            ...
            if not names:
                messagebox.showinfo("提示", "请先在「仅在左」Tab 选中要复制的项", parent=self)
                return
            left, right = _require_dirs()
            if not left or not right:
                return
            _run_copy(model.copy_from_left_to_right, names, left, right)

        def _only_left_copy_from_right():
            names = _get_selected_names(tv_left)
            if not names:
                messagebox.showinfo("提示", "请先在「仅在左」Tab 选中项", parent=self)
                return
            left, right = _require_dirs()
            if not left or not right:
                return
            _run_copy(model.copy_from_right_to_left, names, left, right)
        ...
        def _only_right_copy_left():
            ...
            if not names:
                messagebox.showinfo("提示", "请先在「仅在右」Tab 选中要复制的项", parent=self)
                return
            left, right = _require_dirs()
            if not left or not right:
                return
        ...
        def _only_right_copy_from_left():
            names = _get_selected_names(tv_right)
            if not names:
                messagebox.showinfo("提示", "请先在「仅在右」Tab 选中项", parent=self)
                return
            left, right = _require_dirs()
            if not left or not right:
                return
        ...
        def _diff_copy_right():
            ...
            if not names:
                messagebox.showinfo("提示", "请先在「同名内容不同」Tab 选中项", parent=self)
                return
            left, right = _require_dirs()
            if not left or not right:
                return
        ...
        def _diff_overwrite_right():
            ...
            if not messagebox.askyesno(
                "确认覆盖", f"确定用左目录文件覆盖右目录中选中的 {len(names)} 个文件？", parent=self
            ):
                return
            left, right = _require_dirs()
        ...
        def _diff_overwrite_left():
            ...
            if not names:
                ...
                return
            if not messagebox.askyesno(
                "确认覆盖", f"确定用右目录文件覆盖左目录中选中的 {len(names)} 个文件？", parent=self
            ):
                return
            left, right = _require_dirs()
        ...
        ttk.Button(bottom_frame, text="关闭", command=self.close).pack(side=tk.RIGHT, padx=5)


def show_diff_sync_dialog(app, controller):
    """保持原模块函数接口（Dialogs 路由层调用）。"""
    DiffSyncDialog(app, controller)
