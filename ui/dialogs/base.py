...
def friendly_error(err: object) -> tuple[str, str, str]:
    ...
    if "psi4" in lower and ("basis" in lower or "basis set" in lower):
        return ("基组名字不对 😅", msg, "👉 在下拉框里选一个常见的：6-31g* / def2-svp / cc-pvdz")
    ...
    if "内存" in msg or "memory" in lower:
        return ("内存不够啦 😟", msg, "👉 在参数里把 PSI4 内存调大，或关闭其它占内存的大程序")
    ...
    if "图像" in msg or "image" in lower or "pillow" in lower or "pil" in lower:
        return ("图像模块没装 😟", "做 GIF / 图片预览需要 Pillow", "👉 执行：pip install Pillow")
    if "ffmpeg" in lower:
        return ("没检测到 ffmpeg 😟", "导出 MP4 需要 ffmpeg", "👉 安装后再导出 MP4；GIF 不需要 ffmpeg 可以直接生成")
