...
def show_psi4_dialog(app, controller):
    ...
    runlevel_combo = ttk.Combobox(
        runlevel_grid, textvariable=runlevel_var, values=list(RUN_PRESETS.keys()), state="readonly", width=40
    )
    runlevel_combo.grid(row=0, column=1, padx=5, sticky=tk.W)

    ff_hint_label = ttk.Label(
        frame_runlevel, text="快速模式：会跳过 PSI4，直接使用 MMFF94/UFF 力场优化", foreground="red"
    )
    ...
    preset_combo = ttk.Combobox(
        frame2, textvariable=preset_var, values=list(PSI4_PRESETS.keys()), state="readonly", width=25
    )
    preset_combo.pack(side=tk.LEFT, padx=5)
    ttk.Button(frame2, text="应用", command=lambda: _apply_preset(preset_var, method_var, basis_var)).pack(
        side=tk.LEFT, padx=5
    )
    ...
    scan_mode_menu = ttk.Combobox(
        scan_frame,
        textvariable=scan_mode_var,
    ...
    ttk.Combobox(
        advanced_frame,
        textvariable=solvent_var,
        values=["", "water", "ethanol", "methanol", "acetone", "thf"],
        state="readonly",
        width=10,
    ...
    start_btn = ttk.Button(
        btn_frame,
        text="▶ 开始计算",
        command=lambda: _run_psi4_batch(
            app,
            controller,
            selected,
            task_var,
    ...
    cancel_run_btn = ttk.Button(btn_frame, text="⏹ 取消计算", state="disabled", command=lambda: _request_cancel(app))
...
def _display_scan_result(app, res, result_text):
    if res["success"]:
        ...
        if res.get("pcm_rolled_back"):
            _append_text(app, result_text, "   ⚠️ 扫描中部分帧 PCM 溶剂不可用，已回退为气相（溶剂效应缺失）！\n", "warn")
