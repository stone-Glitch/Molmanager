...
EXAMPLE_MOLECULES: list[dict] = [
    ...
    },
    {
        ...
        "smiles": "c1ccccc1",
        "category": "芳香烃",
        "note": "刚性无转子分子，构象搜索应得 rotor_free=True。",
    },
    {
        "name": "乙酸",
        "english": "aceticacid",
        ...
        "smiles": "CC(=O)O",
        "category": "羧酸",
        "note": "可演示 pKa 热力学循环（HA 与 A- 必须成对）。",
    },
    {
        "name": "咖啡因",
        "english": "caffeine",
...
FAILURE_CASES: list[dict] = [
    ...
    {"title": "中文名映射到两个不同英文名", "why": "反向映射会静默塌缩丢数据——S-06 已加冲突检测并红色告警。"},
    {
        "title": "PCM 溶剂计算被自动回退到气相",
        "why": "溶剂模型不支持时 psi4 会回退气相，S-04/05 已加醒目红字警示，别误当溶剂结果。",
    },
    {"title": "NMR 化学位移凭空出现", "why": "CPHF 不可用时旧版会编造经验位移；S-01 已改为直接拒绝，不会再有假谱。"},
    {"title": "IRC 解析出 0 帧仍画出反应路径", "why": "IRC 无收敛帧时旧版注入 TS 几何造假；S-03 已改为显式报错。"},
]
...
__all__ = ["EXAMPLE_MOLECULES", "FAILURE_CASES", "get_examples", "lookup_example", "get_failure_cases", "categories"]
